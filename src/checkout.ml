    ...
    (fun hdr src () ->
      ...
      let () =
        ...
        | Symbolic ->
            Eio_unix.run_in_systhread ~label:"symlink" (fun () ->
                Unix.symlink hdr.link_name (Eio.Path.native_exn path))
        | _ ->
            Eio.Switch.run @@ fun sw ->
            let dst =
              Eio.Path.open_out ~sw ~append:false
                ~create:(`If_missing file_mode) path
            in
      ...
      Eio_unix.run_in_systhread ~label:"lchown+chmod+utimes" (fun () ->
          ...
          if hdr.link_indicator <> Symbolic then
            Unix.utimes path access_time mod_time))
    fd ()
...
let checkout ?(only_rootfs = false) ~cache ~root i =
  let dir = if only_rootfs then root else root / Image.to_string i in
  Eio.Switch.run @@ fun sw ->
